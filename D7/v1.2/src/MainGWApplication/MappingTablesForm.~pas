...
uses
  ...
  dxSkinWhiteprint, dxSkinXmas2008Blue, cxCheckBox, cxMemo, cxDropDownEdit;
...
type
  TformMappingTables = class(TForm)
    ...
    cxGrid1DBTableView1SyncType: TcxGridDBColumn;
...
const
  ...
  Sync_type : array[0..1] of string =('Data','Track');
...
begin
  ...
  if cdsTables.State in [dsInsert,dsEdit] then cdsTables.Post;
    ...
    frmMappFields.Caption:='Mapping Fields ('+cdsTablestable_src_name.AsString+' <--> '+cdsTablestable_target_name.AsString+')';
...
begin
  ...
  TcxCustomComboBoxProperties(cxGrid1DBTableView1SyncType.Properties).Items.Clear;
  for icount := 0 to length(Sync_type)-1 do
  begin
    TcxCustomComboBoxProperties(cxGrid1DBTableView1SyncType.Properties).Items.Add(Sync_type[icount]);
  end;
end;
