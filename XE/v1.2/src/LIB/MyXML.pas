...
procedure TMyXML.DefaultOnNodeSetName(Sender: TObject; Node: TXmlNode;
  Name: String);
begin
  Node.NodeName := Name;
end;

procedure TMyXML.DefaultOnNodeSetText(Sender: TObject; Node: TXmlNode;
  Text: String);
begin
  Node.Text := Text;
end;
...
function TMyXML.Escape(Value: String): String;
begin
...
function TMyXML.GetText: String;
begin
  Lines.Clear;
  if Ident = '' then
    Lines.LineBreak := '';
...
procedure TMyXML.LoadFromFile(const FileName: String);
var
...
procedure TMyXML.LoadFromStream(const Stream: TStream);
var
...
procedure TMyXML.Parse(Reader: TStreamReader);
var
  Line: String;
...
procedure TMyXML.SaveToFile(const FileName: String);
var
  Stream: TFileStream;
begin
...
procedure TMyXML.SaveToStream(const Stream: TStream);
var
  Encoding: TEncoding;
begin
  GetText;
...
procedure TMyXML.SetText(Value: String);
var
...
function TMyXML.UnEscape(Value: String): String;
begin
  Result := ReplaceStr(Value, '&lt;', '<' );
  Result := ReplaceStr(Result, '&gt;', '>');
  Result := ReplaceStr(Result, '&apos;', chr(39));
...
procedure TMyXML.Walk(Lines: TStringList; Prefix: String;
  Node: TXmlNode);
var
  Child: TXmlNode;
  Attribute: TXmlAttribute;
